    ...
    <div>
      ...
      >
        ...
        <div className="relative mx-auto max-w-3xl px-6 py-28 text-center md:py-40">
          {eyebrow && (
            <p className="text-[11px] font-light uppercase tracking-[0.28em]" style={{ color: "#C9A84C" }}>{eyebrow}</p>
          )}
          <h1 className="mt-8 font-serif text-5xl font-light leading-[1.05] md:text-6xl" style={{ color: "#F5F0E8" }}>{title}</h1>
          {intro && (
            <p className="mx-auto mt-8 max-w-2xl text-base font-light leading-relaxed md:text-lg" style={{ color: "rgba(245,240,232,0.72)" }}>{intro}</p>
          )}
        </div>
      </section>
